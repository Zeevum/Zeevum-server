...
use Zeevum_protocol::{
    AuthMethod, ClientMsg, MAX_MESSAGE_LEN, PROTOCOL_VERSION, ServerMsg, UserBrief, decode, encode,
    pow,
};
...
async fn main() {
    ...
    info!(
        "Zeevum-server v{} starting (protocol v{PROTOCOL_VERSION})",
...
async fn handle_client(
    ...
    let user = match handshake(&mut tls_stream, peer_address, &pool, &fake_hash).await {
        ...
        Err(HandshakeError::Auth(reason)) => {
            ...
            let _ = tls_stream
                .write_all(frame(&ServerMsg::AuthFailed { reason }).as_bytes())
                .await;
    ...
    let (token, expires_at) =
        match db::create_session(&pool, user.id, CONFIG.session_duration_hours).await {
            Ok(t) => t,
            Err(e) => {
                error!("Failed to create session: {e}");
                let _ = tls_stream
                    .write_all(
                        frame(&ServerMsg::AuthFailed {
    ...
    if let Ok(friends) = db::get_friends_list(&pool, &user_id).await {
        let entries = friends
            .into_iter()
            .map(|(chat_id, login)| UserBrief { chat_id, login })
            .collect();
                ...
                match cmd {
                    ...
                    ClientMsg::FriendReq { target_chat_id } => {
                        if let Ok(Some(target)) =
                            db::get_user_by_chat_id(&pool, &target_chat_id).await
                        {
                            let _ = db::add_friend_request(&pool, &user_id, &target.id).await;
                            let _ = hub.send_to(
                                target_chat_id,
                                &frame(&ServerMsg::IncomingReq {
                                    from: UserBrief {
                                        chat_id: user_chat_id,
                            ...
                            let _ = hub.send_to(
                                user_chat_id,
                                &frame(&ServerMsg::Info {
                                    text: "Request sent".into(),
                                }),
                            );
                        }
                    }
                    ...
                    ClientMsg::MarkRead { message_id } => {
                            ...
                            db::mark_message_as_read_checked(&pool, &message_id, &user_id).await
                        {
                            let _ = hub.send_to(
                                sender_chat_id,
                                &frame(&ServerMsg::MsgRead { message_id }),
                            );
                        }
                    }
...
async fn handshake(
    ...
    let method = match method {
        AuthMethod::Register { login, password } => {
            ...
            stream
                ...
                .await
                .map_err(|e| Protocol(format!("Write error: {e}")))?;
            stream
                .flush()
                .await
                .map_err(|e| Protocol(format!("Write error: {e}")))?;
            ...
            let solve_line = tokio::time::timeout(CONFIG.handshake_timeout, read_frame(stream))
                ...
                .map_err(|e| Protocol(format!("Read error: {e}")))?;
            let solved: ClientMsg =
                decode(solve_line.trim()).map_err(|_| Protocol("Malformed PoW frame".into()))?;
    ...
    let user_result = match method {
        AuthMethod::Token { token } => {
            validate_session(pool, &token, CONFIG.session_duration_hours).await
        }
        AuthMethod::Login { login, password } => {
            login_user(pool, &login, &password, fake_hash).await
        }
